...
- **Recovery**: unsaved Mermaid and Excalidraw contents are retained per tab, including untitled drawings and empty scenes, and restored after a reload or unexpected exit. Saving removes the saved recovery entry; closing a dirty tab or confirming exit without saving discards it. Recovery storage failures show an alert so you can save to files. Undo history does not survive a relaunch.
...
- **Click a symbol for its references**: clicking a node in the Mermaid preview (either mode), or a labelled element on the Excalidraw canvas while viewing, opens a panel on the right listing every diagram in the same project that mentions that symbol. Click an entry to activate that document, zoomed to its match - the panel stays open so you can flip through them. Escape clears the highlight first and closes the panel second; ✕ closes it outright. Clicking empty space does nothing, and a document that belongs to no registered project says so instead of listing anything.
- **The symbol you picked follows you**: while the panel is open that symbol is *active*, and it is marked wherever you go - a tab click, `Ctrl+Tab`, `Cmd/Ctrl+1`…9, a file in the sidebar. Diagrams that never mention it simply show nothing. Switching tabs only marks; it never moves the diagram, so each tab stays exactly where you left it (choosing a document in the panel is the one act that zooms to a match). Picking another symbol replaces the active one everywhere, and closing the panel clears the marks from every document you walked through.
...
frontend/       React + Vite frontend (Excalidraw, Mermaid)
  src/components/   Sidebar, toolbar, workspaces, references panel, symbol board, zoom/pan viewport
  src/hooks/        Document tabs and modes, the Excalidraw/Mermaid engines, the symbol index
  src/lib/          Tauri command wrappers, Mermaid conversion, symbol extraction + picking, board thumbnails, text refit
